...
**哪些版本提供下载入口：只有「最新一版」加上生成器里的 `PINNED_DOWNLOADS`（目前是 1.0.1）。**
其余版本不生成链接，展开后只有一条静态的「此版本不提供下载，请用最新版」——
旧版正是带 bug 的那几版，别把用户顺着列表引到旧安装包上去。每发一版，
上一版会自动失去入口，不用改脚本；想让某个历史版本重新可下，把它加进
`PINNED_DOWNLOADS` 再跑一次生成器。

**日期不写进文件** —— 它由页面打开时的 `docs/assets/app.js` 从 GitHub 现场取
（一次请求，结果在浏览器里缓存 30 分钟）。所以：

- 忘了跑生成脚本 → 列表里少一版，但日期仍然是对的；
- 最新版 / 钉住版还没建 Release → 那一版后面显示「未单独发布安装包」，不会有 404 链接；
- 没网或被 GitHub 限流（未认证额度 60 次/小时）→ 日期整个藏起来，不会留一排「—」。

### 下载统计在哪看

**公开页面上不显示任何下载次数**（zed 的要求：统计只给自己看）。想看数字：
首页**右下角**有个几乎隐形的「···」小圆点（悬停才会明显），点开输入口令，
展开的统计面板里有 .dmg / .zip 累计和每个版本的下载次数。

- 口令在 `docs/assets/app.js` 的 `STATS_PIN`（明文常量，改它就换口令）；
  通过一次后在浏览器里记住，不再重复问。
- 口令只是一道帘子，**不是安全边界** —— 数据本身在 GitHub API 上是公开的。
- 面板按 Release 展示；像 1.1.0 这种没建过 Release 的版本号不在面板里（本来也没有数字）。
...
**这不是 bug，是设计**：公开页面上不显示下载次数（统计只给自己看）。数字都在
右下角那个「···」小圆点的统计面板里 —— 输入口令就能看，见上面「下载统计在哪看」。

- 面板里每次访问只发一个请求，结果在浏览器里存 30 分钟；被限流时会退回上次存下来的
  数字，实在一次都没成功过才会显示「取不到数据」。
- 次数是 GitHub 自己统计的附件下载量，**不是唯一的访客数**（命令行、镜像、重复下载都算在内）。

### 统计面板里少了某个版本号

面板按 **Release** 列表展示。只改了 `CHANGELOG.md`、没建对应 Release 的版本号
（比如 1.1.0）不在面板里 —— 本来也没有数字。想让数字出现就去补一个 tag + Release。
...
└── assets/
    ...
    ├── app.js         主题切换、Release 版本号/体积/日期自动填充、
    │                  站长统计面板（页脚角落 + 口令）、复制按钮、移动端导航
